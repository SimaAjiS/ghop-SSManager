...
**背景**
Issue 11 でエクセルエクスポート機能が実装され、テンプレートファイルを使用してデータを出力する機能が追加された。しかし、実際の出力結果において、データの行ずれが発生したり、テンプレートで意図したレイアウトが再現されない問題が確認されていた。
...
- **ヘッダー情報**:
  - Sheet No: J5（K3 から変更）
  - Sheet Revision: N5（O3 から変更）
  - Sheet Name: D7（変更なし）

- **Chip Specs**:
  - Chip Size: L8（H8 から変更）
  - Pad Gate: L10（変更なし）
  - Pad Source: L11（変更なし）
  - Dicing Line: L12（H12 から変更）
  - PDPW: L16（H16 から変更）
...
### 2. Wafer Probing Spec の改善

- **開始位置**: 25行目（変更なし）
- **列マッピング**: No(C), Item(D), Min(F), Typ(G), Max(H), Unit(I), Cond(J)
- **CONDITION フォーマット**: フロントエンドと同様のロジックを実装
  - `VGS=...`, `VDS=...` などの形式で複数のバイアス条件を結合
  - 空白フィールドは自動的にスキップ
- **動的行挿入**: データ量に応じて自動的に行を挿入

### 3. Related Devices セクションの修正

- **開始位置**: 49行目（40行目から変更）
- **列マッピング**: Type(F), Top Metal(I), Wafer Thickness(K), Back Metal(M)
  - 以前: Type(C), Top Metal(F), Wafer Thickness(I), Back Metal(L)
- **動的行挿入**: データ量に応じて自動的に行を挿入

### 4. 追加フィールドの実装

- **ESD**: D36（Wafer Probing Spec の行挿入に応じてシフト）
- **Sheet Name**: G48（Wafer Probing Spec と Related Devices の行挿入に応じてシフト）
- **更新日**: M53（すべての行挿入に応じてシフト）

### 5. Chip Appearance 画像の挿入

- **セル位置**: C9
- **画像処理**:
  - `MT_maskset.appearance` フィールドに基づいて画像を挿入
  - 画像が存在しない場合は自動生成された "No Image" プレースホルダーを使用
  - 最大サイズ 300px に自動リサイズ
- **依存関係**: `Pillow` ライブラリを追加

### 6. テンプレートファイルの変更

- **旧テンプレート**: `specsheet_template.xlsm`（マクロ有効）
- **新テンプレート**: `specsheet_template.xlsx`（標準形式）
- **Git 管理**: 新テンプレートを Git 管理対象に追加（強制追加）

### 7. ファイル名形式の変更

- **新形式**: `[sheet_no]_[sheet_name].xlsx`
  - 例: `G05144A_6KT18B2N2A.xlsx`
- **フォールバック**: `sheet_no` または `sheet_name` が空の場合は "X" を使用
- **CORS 設定**: `Content-Disposition` ヘッダーを公開してフロントエンドがファイル名を読み取れるように設定

**完了したタスク**

- [x] 現在のエクスポート処理のコードを詳細に調査
- [x] テンプレートファイルの構造とデータ挿入位置の確認
- [x] データマッピングロジックの見直しと修正
- [x] 行挿入処理の動作確認と修正
- [x] セクションごとのデータ配置の検証
- [x] CONDITION フィールドのフォーマット改善
- [x] Chip Appearance 画像挿入機能の実装
- [x] テンプレートファイルの .xlsx 形式への変更
- [x] ファイル名形式の変更
- [x] 実データでの出力結果の確認と比較
- [x] テンプレートレイアウトとの整合性確認
- [x] 各種デバイスタイプでの動作検証

**技術的な改善点**

- `openpyxl.drawing.image.Image` を使用した画像挿入
- 動的な行挿入ロジックによる柔軟なレイアウト対応
