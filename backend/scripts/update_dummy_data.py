...
def anonymize_string(value, prefix_pattern=None):
    ...
    if prefix_pattern:
        ...
        # 6Pで始まるシート名（例: 6PT1603N1T**）
            ...
            # パターンに一致しない場合は少し変更
            if len(value_str) > 3:
                return value_str[:3] + str(random.randint(100, 999)) + value_str[6:] if len(value_str) > 6 else value_str
            return value_str
        ...
        # GK, GP, GXで始まる（例: GP016A）
            ...
            if len(value_str) >= 5:
                ...
                try:
                    ...
                    return value_str[:2] + f"{random.randint(800, 999)}{value_str[5:] if len(value_str) > 5 else ''}"
    ...
    if ' :' in value_str or ':' in value_str:
        ...
        if len(parts) == 2:
            try:
                num = int(parts[0])
                new_num = num + random.randint(1, 10) if num > 0 else random.randint(1, 10)
...
def anonymize_number(value, variation_percent=5):
    ...
    try:
        ...
        return round(num + random.uniform(-variation, variation), 2) if isinstance(value, float) else int(num + random.uniform(-variation, variation))
    except:
        return value
...
def update_dummy_data():
    ...
    if os.path.exists(dummy_file):
        try:
            ...
            for sheet_name in dummy_xls.sheet_names:
                existing_data[sheet_name] = pd.read_excel(dummy_xls, sheet_name=sheet_name)
    ...
    with pd.ExcelWriter(dummy_file, engine='openpyxl') as writer:
        for sheet_name in template_sheets:
            ...
            print(f"  Template has {len(template_df)} rows, {len(template_df.columns)} columns")
            ...
            for col in dummy_df.columns:
                ...
                if col == '+/-' or col == '±':
                    ...
                    dummy_df[col] = dummy_df[col].apply(lambda x: not x if pd.notna(x) and isinstance(x, bool) else x)
                    continue

                # 数値カラム
                if any(keyword in col_lower for keyword in ['_v', '_a', '_um', '_mm', 'thickness', 'tolerance', 'revision', 'pdpw', 'dicing', 'chip_x', 'chip_y', 'pad_']):
                    try:
                ...
                # 文字列カラム（シート番号、名前など）
                elif any(keyword in col_lower for keyword in ['sheet_no', 'sheet_name', 'type', 'maskset', 'barrier', 'top_metal', 'passivation', 'back_metal', 'status', 'item', 'unit', 'esd', 'appearance', 'level', 'description', 'display', 'cond', 'bias_', 'si_prefix', 'unit_category']):
                    try:
                        dummy_df[col] = dummy_df[col].apply(lambda x: anonymize_string(x, prefix_pattern=True))
                    except Exception as e:
                        print(f"    Warning: Error processing string column '{col}': {e}")

                # その他の文字列
                else:
                    try:
                        dummy_df[col] = dummy_df[col].apply(lambda x: anonymize_string(x) if isinstance(x, str) else x)
                    except Exception as e:
