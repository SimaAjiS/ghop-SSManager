...
mt_device = Table(
    ...
    Column(
        "id", BigInteger
    ),  # Not PK, so BigInteger is fine, but if we want it to be auto-filled, it needs to be handled.
    ...
    ),  # Changed from BigInteger to String to match Excel data format
    Column("back_metal", String),
...
mt_maskset = Table(
    ...
    Column(
        "pdpw", String
    ),  # Changed from Integer to String to handle comma-separated values like "23,318"
