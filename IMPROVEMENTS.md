...
- `backend/app/schema.py` と Pydantic モデルでカラム型・主キー・FK を厳密に定義しています。
- `backend/app/scripts/import_data.py` では Excel 取り込み時に型変換・NaN 正規化・FK チェックを実施し、警告としてログに残します。
...
- Excel テンプレートの NOTE セクションは 12 行までしか出力できず、関連機種が 13 件以上存在する場合はエラーとして弾いている。
- **課題**: 追加行や複数ページ構成など、テンプレート側のレイアウト拡張を検討する必要がある。
